...
unsigned char *h_imageBitmap = NULL;
...
int fpsCount = 0;
int fpsLimit = 15;
unsigned int timesRendered = 0;
int imageW = 3840, imageH = 2160;
...
//	h_imageBitmap2 = (unsigned char *)malloc(w * h * 4);
//
...
void renderImage(bool justCopy)
{
	...
	if (timesRendered % FRAMEDIFF != 0 && !justCopy)
		return;

	pngNumber++;
	checkCudaErrors(cudaMemcpy(h_imageBitmap, d_imageBitmap, 3 * imageW*imageH, cudaMemcpyDeviceToHost));
	...
	stbi_write_png(fileName, imageW, imageH, 3, (const void *)h_imageBitmap, imageW * 3);
...
int main(int argc, char **argv)
{
	...
	h_imageBitmap = (unsigned char *)malloc(imageW * imageH * 3);
	checkCudaErrors(cudaMalloc((void **)&d_imageBitmap, imageW * imageH * 3));

	renderImage(false);//pierwsza klatka
